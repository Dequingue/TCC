...
ALTER TABLE USUARIO ADD avatar VARCHAR(255) NULL;
ALTER TABLE USUARIO ADD criado_em TIMESTAMP DEFAULT CURRENT_TIMESTAMP;
...
INSERT INTO USUARIO(CPF, nome_usuario, email_usuario, dtNascimento, plano_tratamento,senha_usuario) values (111, 'Matheus','joaoluiza6@gmail.com','2000/10/02','Premium',123456);

DESCRIBE USUARIO;
select* from USUARIO ;

update USUARIO
set senha_usuario = 123456
where CPF = 111;
...
CREATE TABLE PROFISSIONAIS (
  ...
  FOREIGN KEY (ESPECIALIDADE_especialidade_PK) REFERENCES ESPECIALIDADE(especialidade_PK),
  FOREIGN KEY (AREA_ATUACAO_areaAtuacao_PK) REFERENCES AREA_ATUACAO(areaAtuacao_PK)
);
...
MODIFY COLUMN relato_consulta VARCHAR(255) NULL;

CREATE TABLE CONSULTA (
  idCONSULTA INT UNSIGNED NOT NULL AUTO_INCREMENT,
  PROFISSIONAIS_idProfissional INT UNSIGNED NOT NULL,
  USUARIO_idUsuario INT NOT NULL, -- sem UNSIGNED
  data_consulta DATE NOT NULL,
  hora_consulta TIME NOT NULL,
  relato_consulta VARCHAR(255) NULL,
  nota_consulta VARCHAR(255) NULL,
  observacao_consulta VARCHAR(255) NULL,
  PRIMARY KEY (idCONSULTA),
  INDEX idx_usuario(USUARIO_idUsuario),
  INDEX idx_profissional(PROFISSIONAIS_idProfissional),
  FOREIGN KEY (PROFISSIONAIS_idProfissional)
  ...
  FOREIGN KEY (USUARIO_idUsuario)
    ...
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT UC_ConsultaUnica UNIQUE (PROFISSIONAIS_idProfissional, USUARIO_idUsuario, data_consulta, hora_consulta)
);
